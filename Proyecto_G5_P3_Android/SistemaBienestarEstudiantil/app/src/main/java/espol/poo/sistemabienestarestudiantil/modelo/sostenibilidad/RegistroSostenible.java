...
public class RegistroSostenible implements Serializable {
    ...
    // Lógica para contar cuántas veces se ha realizado cada acción
    ...
    public int contarDiasConAcciones() {
        int count = 0;
        for (RegistroDiarioSostenible r : registros) {
            if (!r.getAccionesSeleccionadasIds().isEmpty()) count++;
        }
        return count;
    }
    ...
    public int contarDiasPerfectos(int totalAcciones) {
        int count = 0;
        for (RegistroDiarioSostenible r : registros) {
            if (r.getAccionesSeleccionadasIds().size() == totalAcciones) count++;
        }
        return count;
    }
}